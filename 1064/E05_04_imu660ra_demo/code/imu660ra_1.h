...
//====================================================软件 IIC 驱动====================================================
#define IMU660RA_SOFT_IIC_DELAY_1    (100)                                       // 软件 IIC 的时钟延时周期 数值越小 IIC 通信速率越快
#define IMU660RA_SCL_PIN_1            (C23)                                       // 软件 IIC SCL 引脚 连接 IMU660RA 的 SCL 引脚
#define IMU660RA_SDA_PIN_1            (C22)                                       // 软件 IIC SDA 引脚 连接 IMU660RA 的 SDA 引脚
//====================================================软件 IIC 驱动====================================================
#else

//====================================================硬件 SPI 驱动====================================================
#define IMU660RA_SPI_SPEED_1          (10 * 1000 * 1000)                          // 硬件 SPI 速率
#define IMU660RA_SPI_1                (SPI_4)                                     // 硬件 SPI 号
#define IMU660RA_SPC_PIN_1            (SPI4_SCK_C23 )                             // 硬件 SPI SCK 引脚
#define IMU660RA_SDI_PIN_1            (SPI4_MOSI_C22)                             // 硬件 SPI MOSI 引脚
#define IMU660RA_SDO_PIN_1            (SPI4_MISO_C21)                             // 硬件 SPI MISO 引脚
//====================================================硬件 SPI 驱动====================================================
#endif
#define IMU660RA_CS_PIN_1             (C20)                                       // CS 片选引脚
#define IMU660RA_CS_1(x)              ((x) ? (gpio_high(IMU660RA_CS_PIN)) : (gpio_low(IMU660RA_CS_PIN)))
    
#define IMU660RA_TIMEOUT_COUNT_1      (0x00FF)                                    // IMU660 超时计数


//================================================定义 imu660ra 内部地址================================================
#define IMU660RA_DEV_ADDR_1           (0x69)                                      // SA0接地：0x68 SA0上拉：0x69 模块默认上拉
...
#define IMU660RA_ACC_SAMPLE_1        (0x02)                                      // 加速度计量程
...
#define IMU660RA_GYR_SAMPLE_1         (0x00)                                      // 陀螺仪量程
...
extern int16 imu660ra_gyro_x_1, imu660ra_gyro_y_1, imu660ra_gyro_z_1;                   // 三轴陀螺仪数据      gyro (陀螺仪)
extern int16 imu660ra_acc_x_1, imu660ra_acc_y_1, imu660ra_acc_z_1;                      // 三轴加速度计数据     acc (accelerometer 加速度计)


void  imu660ra_get_acc_1           (void);                                         // 获取 IMU660RA 加速度计数据
void  imu660ra_get_gyro_1          (void);                                         // 获取 IMU660RA 陀螺仪数据
float imu660ra_acc_transition_1     (int16 acc_value);                              // 将 IMU660RA 加速度计数据转换为实际物理数据
float imu660ra_gyro_transition_1   (int16 gyro_value);                             // 将 IMU660RA 陀螺仪数据转换为实际物理数据
uint8 imu660ra_init_1             (void);                                         // 初始化 IMU660RA
