...
#include "imu660ra_8.h"

int16 imu660ra_gyro_x_8 = 0, imu660ra_gyro_y_8 = 0, imu660ra_gyro_z_8 = 0;                           // 三轴陀螺仪数据      gyro (陀螺仪)
int16 imu660ra_acc_x_8 = 0, imu660ra_acc_y_8 = 0, imu660ra_acc_z_8 = 0;                              // 三轴加速度计数据     acc (accelerometer 加速度计)
...
// 备注信息     内部调用
//-------------------------------------------------------------------------------------------------------------------
static uint8 imu660ra_self_check_8 (void)
{
    uint8 dat = 0, return_state = 0;
    uint16 timeout_count = 0;
    do
    {
        if(timeout_count ++ > IMU660RA_TIMEOUT_COUNT_8)
        {
            return_state =  1;
            break;
        }
        dat = imu660ra_read_register(IMU660RA_CHIP_ID_8);
        system_delay_ms(1);
...
//-------------------------------------------------------------------------------------------------------------------
void imu660ra_get_acc_8 (void)
{
    uint8 dat[6];

    imu660ra_read_registers(IMU660RA_ACC_ADDRESS_8, dat, 6);
    imu660ra_acc_x_8 = (int16)(((uint16)dat[1]<<8 | dat[0]));
    imu660ra_acc_y_8 = (int16)(((uint16)dat[3]<<8 | dat[2]));
    imu660ra_acc_z_8 = (int16)(((uint16)dat[5]<<8 | dat[4]));
}
//-------------------------------------------------------------------------------------------------------------------
...
//            使用 IIC 的采集时间为126us
//-------------------------------------------------------------------------------------------------------------------
void imu660ra_get_gyro_8 (void)
{
    uint8 dat[6];

    imu660ra_read_registers(IMU660RA_GYRO_ADDRESS_8, dat, 6);
    imu660ra_gyro_x_8 = (int16)(((uint16)dat[1]<<8 | dat[0]));
    imu660ra_gyro_y_8 = (int16)(((uint16)dat[3]<<8 | dat[2]));
    imu660ra_gyro_z_8 = (int16)(((uint16)dat[5]<<8 | dat[4]));
}
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float imu660ra_acc_transition_8 (int16 acc_value)
{
    float acc_data = 0;
    switch(IMU660RA_ACC_SAMPLE_8)
    {
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
float imu660ra_gyro_transition_8 (int16 gyro_value)
{
    float gyro_data = 0;
    switch(IMU660RA_GYR_SAMPLE_8)
    {
...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
uint8 imu660ra_init_8 (void)
{
    uint8 return_state = 0;
    system_delay_ms(20);                                                    // 等待设备上电成功
#if IMU660RA_USE_SOFT_IIC
    soft_iic_init(&imu660ra_iic_struct, IMU660RA_DEV_ADDR_8, IMU660RA_SOFT_IIC_DELAY_8, IMU660RA_SCL_PIN_8, IMU660RA_SDA_PIN_8);        // 配置 IMU660RA 的 IIC端口
#else
...
#endif
    do{
        if(imu660ra_self_check_8())                                           // IMU660RA自检
        {
        ...
        imu660ra_write_register(IMU660RA_PWR_CONF_8, 0x00);                   // 关闭高级省电模式
        system_delay_ms(1);
        imu660ra_write_register(IMU660RA_INIT_CTRL_8, 0x00);                  // 开始对模块进行初始化配置
        imu660ra_write_registers(IMU660RA_INIT_DATA_8, imu660ra_config_file, sizeof(imu660ra_config_file));                       // 输出配置文件
        imu660ra_write_register(IMU660RA_INIT_CTRL_8, 0x01);                  // 初始化配置结束
        system_delay_ms(20);
        if(imu660ra_read_register(IMU660RA_INT_STA_8) == 0)                   // 检查是否配置完成
        {
            // 如果程序在输出了断言信息 并且提示出错位置在这里
        ...
        imu660ra_write_register(IMU660RA_PWR_CTRL_8, 0x0E);                   // 开启性能模式  使能陀螺仪、加速度、温度传感器
        imu660ra_write_register(IMU660RA_ACC_CONF_8, 0xA7);                   // 加速度采集配置 性能模式 正常采集 50Hz采样频率
        imu660ra_write_register(IMU660RA_GYR_CONF_8, 0xA9);                   // 陀螺仪采集配置 性能模式 正常采集 200Hz采样频率
        imu660ra_write_register(IMU660RA_ACC_RANGE_8, IMU660RA_ACC_SAMPLE_8);   // 加速度量程配置 配置量程为:±8g
        imu660ra_write_register(IMU660RA_GYR_RANGE_8, IMU660RA_GYR_SAMPLE_8);   // 陀螺仪量程配置 配置量程为:±2000dps
